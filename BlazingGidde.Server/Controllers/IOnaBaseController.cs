...
public interface IOnaBaseController<TEntity, Tkey, TDataContext, TReadDto, TCreateDto, TUpdateDto, TCreateDtoResponse,
    TUpdateDtoReponse>
    ...
    where TUpdateDtoReponse : class
{
    ...
    Task<ActionResult<APIEntityResponse<QueryFilterResponse<TReadDto>>>>
        GetWithFilter(QueryFilter<TEntity> queryFilter);

    Task<ActionResult<APIEntityResponse<QueryFilterResponse<TReadDto>>>> GetWithLinqFilter(
        LinqQueryFilter<TEntity> linqQueryFilter);
