	...
	public class UserRoleController : ControllerBase
	{
		...
		public async Task<ActionResult<APIResponse>> AssignRole([FromBody] AssignRoleDto dto)
		{
			...
			try
			{
				...
				if (success)
				{
					_logger.LogInformation("Role '{RoleName}' assigned to user with ID: {UserId} successfully.", dto.RoleName, dto.UserId);
				...
				_logger.LogWarning("Failed to assign role '{RoleName}' to user with ID: {UserId}.", dto.RoleName, dto.UserId);
				return BadRequest(new APIResponse
				{
					Success = false,
			...
			{
				_logger.LogError(ex, "An error occurred while assigning role '{RoleName}' to user with ID: {UserId}.", dto.RoleName, dto.UserId);
		...
		public async Task<ActionResult<APIResponse>> RemoveRole([FromBody] AssignRoleDto dto)
		{
			...
			try
			{
				...
				if (success)
				{
					_logger.LogInformation("Role '{RoleName}' removed from user with ID: {UserId} successfully.", dto.RoleName, dto.UserId);
					return Ok(new APIResponse
					{
						Success = true,
					});
				...
				_logger.LogWarning("Failed to remove role '{RoleName}' from user with ID: {UserId}.", dto.RoleName, dto.UserId);
				return BadRequest(new APIResponse
				{
					Success = false,
			...
			{
				_logger.LogError(ex, "An error occurred while removing role '{RoleName}' from user with ID: {UserId}.", dto.RoleName, dto.UserId);
				return StatusCode(StatusCodes.Status500InternalServerError, new APIResponse
				{
					Success = false,
