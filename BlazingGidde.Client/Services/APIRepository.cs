    ...
    where TUpdateDtoResponse : class
{
    ...
    public async Task<QueryFilterResponse<TReadDto>> Get(QueryFilter<TEntity> queryFilter)
    {
        try
        {
            ...
            var apiResponse = JsonSerializer.Deserialize<APIEntityResponse<QueryFilterResponse<TReadDto>>>(responseBody,
                new JsonSerializerOptions
                {
                    PropertyNameCaseInsensitive = true
                });

            if (apiResponse?.Success == true) return apiResponse.Items!;
