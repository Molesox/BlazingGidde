    ...
    where TEntity : class
{
    ...
    /// <summary>
    ///     Applies the filter, ordering, includes, and paging to the query and returns the resulting
    ...
    public IQueryable<TEntity> GetFilteredList(IQueryable<TEntity> query)
    {
        ...
        // Apply the filter
        if (deserializedExpression != null) query = query.Where(deserializedExpression);
